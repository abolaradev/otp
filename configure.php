...
function makeAbolaradevCLI()
{
    ...
    passthru(
        PHP_BINARY.' '.escapeshellarg(__DIR__.'/vendor/bin/testbench')
        .' '.implode(' ', array_map('escapeshellarg', array_slice($argv, 1)))
    );
    php;

    $file = fopen('abolaradev','w');
    fwrite($file,$content);
    fclose($file);
}
...
if(!is_dir(__DIR__."\\workbench") && file_exists(__DIR__."\\workbench.zip")){
    extractWorkbench();
}
...
if(!file_exists(__DIR__."\\abolaradev")){
    makeAbolaradevCLI();
}
